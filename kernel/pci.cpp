...
namespace {
  ...
  Error ConfigureMSIRegister(const Device& dev, uint8_t cap_addr,
    ...
    if (msi_cap.header.bits.multi_msg_capable <= num_vector_exponent) {
      msi_cap.header.bits.multi_msg_enable = msi_cap.header.bits.multi_msg_enable;
    } else {
...
namespace pci{
  // WriteAddressを用いてCONFIG_ADDRESSを設定して，WriteDataとReadDataを用いてPCIコンフィギュレーション空間にアクセス
  ...
  Error ScanAllBus() {
    ...
    // ホストブリッジにマルチファンクションの場合
    // ファンクション番号がそれぞれのバスを担当
    for (uint8_t function = 1; function < 8; ++function) {
